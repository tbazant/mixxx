...
NS4FX.init = function(id, debug) {
    ...
    NS4FX.effectUnit.effects.forEach(function(effect) {
        const button = NS4FX.effectUnit.effectButtons[effect.name];
...
NS4FX.EffectUnit = function() {
    ...
    this.leftSwitch = function(_channel, _control, value, _status, _group) {
        const active = value !== 0x00;
        const inactiveDeck = this.deck1 ? 3 : 1;
        const activeDeck = this.deck1 ? 1 : 3;
        const isActive = deck => (deck === activeDeck) || !OnlyActiveDeckEffect;
    ...
    this.rightSwitch = function(_channel, _control, value, _status, _group) {
        const active = value !== 0x00;
        const inactiveDeck = this.deck2 ? 4 : 2;
        const activeDeck = this.deck2 ? 2 : 4;
        const isActive = deck => (deck === activeDeck) || !OnlyActiveDeckEffect;
...
NS4FX.Deck = function(number, midi_chan) {
    ...
    this.orientation = new components.Button({
        ...
        input: function(_channel, _control, value, _status) {
            const desiredOrientation = [1, 0, 2][value];
    ...
    this.cue_button = new components.CueButton({
        ...
        input: function(channel, control, value, status, group) {
            ...
            const isShiftedPress = (control === (this.midi[1] + this.shiftOffset));
    ...
    this.sampler_buttons = new components.ComponentContainer({
        updateLEDs: function() {
            for (const button in deck.sampler_buttons) {
    ...
    const addShiftClearToHotcue = function(button) {
        const original_input = button.input;
        ...
        this.autoloop_buttons[5 - i] = new components.Button({
            ...
            input: function(_channel, _control, value, _status) {
                ...
                if (value === 0x7F) { // Button pressed
                    ...
                    const loopLength = Math.pow(2, 5 - this.number); // Button 1 = 1 beat, Button 2 = 2 beats, etc.
    ...
    this.pitch = new components.Pot({
        ...
        inSetParameter: function(value) {
            ...
            const processedValue = 1 - value;

            let calculatedValue = this.outMin + processedValue * (this.outMax - this.outMin);

            if (this.inKey === "rateRange") {
                const originalCalculatedValue = calculatedValue;
    ...
    const pitch_button_handler = function(channel, control, value, status, _group) {
    ...
    const key_up_or_down = function(channel, control, value, status, _group) {
    ...
    this.padMode = new components.ComponentContainer({
        ...
        turnOffOtherButtons: function(activeButton) {
            for (const button in this) { // NOSONAR
            ...
            loop_toggle: new components.Button({
                ...
                input: function(_channel, _control, value, _status) {
                    if (value === 0x7F) { // Button pressed
                        ...
                        const currentPosition = engine.getValue(this.group, "playposition");
...
NS4FX.encodeNumToArray = function(number) {
    ...
    const absNumber = isNegative ? -number : number;
...
NS4FX.sendScreenPitchMidi = function(deck, rate) {
    ...
    let encoded = 0x800000 + valueToSend;
    ...
    const bytePostfix = [0xF7];
    const byteArray = bytePrefix.concat(pitchArray, bytePostfix);
...
NS4FX.wheelTurn = function(channel, _control, value, _status, group) {
    ...
    // to be
    const delta = Math.abs(NS4FX.last_scratch_tick[deck] - value);
    ...
    if (displayVUFromBothDecks) {
        ...
        if (group === "[Main]") {
            if (control === "vu_meter_left") {
                ...
                // Send to the active meter on the left side (Deck 1 or 3).
                const left_midi_chan = NS4FX.effectUnit.deck1 ? 0xB0 : 0xB2;
                ...
                const right_midi_chan = NS4FX.effectUnit.deck2 ? 0xB1 : 0xB3;
        ...
        const combined_value = Math.max(NS4FX.vu_levels[group].left, NS4FX.vu_levels[group].right);
        ...
        const deckNum = parseInt(group.charAt(8));
        if (NS4FX.decks[deckNum].active) {
            const midiChan = 0xB0 + NS4FX.decks[deckNum].midi_chan;
...
NS4FX.setArrow = function(deckNumber, direction) {
    const midi_chan = NS4FX.decks[deckNumber].midi_chan;
    const upArrowNote = 0x09;
    const downArrowNote = 0x0A;
...
NS4FX.updateBpmArrows = function(deckNumber) {
    ...
    const roundedDeckBpm = Math.round(deckBpm * 10) / 10;
    const roundedOppositeDeckBpm = Math.round(oppositeDeckBpm * 10) / 10;

    // A rate_dir of -1.0 means "down" on the pitch slider increases the track's speed (BPM).
    const downIncreasesSpeed = (engine.getValue(deckGroup, "rate_dir") === -1.0);
